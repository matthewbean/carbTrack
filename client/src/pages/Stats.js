  ...
  const today = new Date();
    ...
    return (
        <div id = "graph">
          ...
          <input className = "m-2"  checked = {check.fat} onChange = {onCheck} type="checkbox" name="fat" value="fat"/> <label className = "mr-auto" htmlFor = "fat"> fat</label>
          ...
          {check.carbs && check.goals && <Line type="monotone" dataKey="carbsGoal" stroke="#8884d8" /> }
          {check.fat &&<Line type="monotone" dataKey="fat" stroke="#000" />}
          {check.fat && check.goals && <Line type="monotone" dataKey="fatGoal" stroke="#000" />}
